...
shell.echo(chalk.green(`Copying package.json, LICENSE.md, README.md into ${NPM_DIR}`));
shell.cp(`-Rf`, [`package.json`, `LICENSE.md`, `README.md`], `${NPM_DIR}`);

/* REMOVE SOURCE CODE AND TEMP FILES */
shell.echo(chalk.green(`Removing unnessary files`));
...
/* MOVE DIST/LIB OUTSIDE */
shell.echo(chalk.green(`Flattening dist/lib`));
shell.cp(`-Rf`, [`${NPM_DIR}/*`], './');
shell.rm(`-Rf`, `./dist`);
